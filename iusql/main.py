...
from .uptycs_restcall import OperationalError
...
from .lexer import IusqlCliLexer
...
class IusqlCli(object):
    ...
    def __init__(
        ...
        self.apifile_data = json.load(open(keyfile))
        self.customer_name = self.apifile_data['domain']
        self.domain = self.customer_name
        self.suffix = ".uptycs.io"
        if suffix:
            self.suffix = suffix
        self.url = "https://" + self.domain + self.suffix
        self.customer_id = self.apifile_data['customerId']
        self.key = self.apifile_data['key']
        self.secret = self.apifile_data['secret']
        self.sqlexecute = sqlexecute
        self.logfile = logfile
        self.verify_ssl = verify_ssl

        # Load config.
        c = self.config = get_config(iusqlrc)
    ...
    def change_db(self, arg, **_):
        if arg is None:
            ...
            if arg.lower() not in ['global', 'realtime','audit']:
                self.sqlexecute.connect(database=self.sqlexecute.dbname)
            else:
    ...
    def initialize_logging(self):
        ...
        root_logger = logging.getLogger("iusql")
        root_logger.addHandler(handler)
        root_logger.setLevel(level_map[log_level.upper()])

        logging.captureWarnings(True)

        root_logger.debug("Initializing iusql logging.")
    ...
    def connect(self, database=""):
        ...
        def _connect():
            self.sqlexecute = SQLExecute(self.url,
                                         self.customer_id,
                                         self.key,
                                         self.secret,
                                         database,
    ...
    def run_cli(self):
        ...
        if not self.less_chatty:
            ...
            print("Uptycs Security Solution")
            print("Uptysc Inc.")
        ...
        def get_continuation(width, line_number, is_soft_wrap):
            if self.prompt_continuation_format:
                self.logger.debug(self.prompt_continuation_format)
                plen = len(self.prompt_continuation_format)
                continuation = (" " * (width - plen - 1) +
        ...
        with self._completer_lock:
            ...
            self.prompt_app = PromptSession(
                lexer=PygmentsLexer(IusqlCliLexer),
    ...
    def get_prompt(self, string):
        ...
        sqlexecute = self.sqlexecute
        now = datetime.now()
        string = string.replace('\\U', self.customer_name)
        string = string.replace('\\t', 'iusql')
...
@click.command()
@click.option("-V", "--version", is_flag=True, help="Output iusql's version.")
@click.option("-D", "--database", "dbname", help="Database to use.")
@click.option(
    "-R",
    "--prompt",
    "prompt",
    help='Prompt format (Default: "{0}").'.format(IusqlCli.default_prompt),
)
@click.option(
    "-l",
    "--logfile",
...
@click.option("-k", "--keyfile", "keyfile", help="Uptycs Key file.")
@click.option("--domainsuffix","domainsuffix",help="Uptycs Domain Suffix like .uptycs.io")
@click.option("--nossl", is_flag=True, help="do not verify ssl certificate")
...
def cli(
    ...
    if not keyfile:
        print("Keyfile and dbname required to continue\nPlease check --option")
        sys.exit(1)
    ...
    if (not database and not dbname):
        print("ERROR: missing database")
        sys.exit(1)

    database = database or dbname

    if (database not in ['global','realtime','audit']):
        print("ERROR: database doesn't exists")
        sys.exit(1)
    ...
    iusql.connect(database)

    iusql.logger.debug("Launch Params: \n" "\tdatabase: %r", database)

    #  --execute argument
    if execute:
        try:
            if csv:
                iusql.formatter.format_name = "csv"
            elif not table:
                iusql.formatter.format_name = "tsv"

            iusql.run_query(execute)
            exit(0)
        except Exception as e:
    ...
    if sys.stdin.isatty():
        ...
        try:
            ...
            iusql.logger.warning("Unable to open TTY as stdin.")

        if (
            iusql.destructive_warning
        ...
        try:
            ...
            iusql.run_query(stdin_text, new_line=new_line)
            exit(0)
        except Exception as e:
            click.secho(str(e), err=True, fg="red")
            exit(1)
