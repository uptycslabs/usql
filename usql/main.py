...
from .lexer import UsqlCliLexer
...
class UsqlCli(object):
    ...
    def __init__(
        ...
        # Load config.
        c = self.config = get_config(usqlrc)
    ...
    def change_db(self, arg, **_):
        if arg is None:
            ...
            if arg.lower() not in ["global", "realtime", "timemachine"]:
    ...
    def initialize_logging(self):
        ...
        root_logger = logging.getLogger("usql")
        root_logger.addHandler(handler)
        root_logger.setLevel(level_map[log_level.upper()])

        logging.captureWarnings(True)

        root_logger.debug("Initializing usql logging.")
    ...
    def run_cli(self):
        ...
        def get_continuation(width, line_number, is_soft_wrap):
            if self.prompt_continuation_format:
                ...
                continuation = (
                    " " * (width - plen - 1) + self.prompt_continuation_format + " "
                )
            else:
        ...
        with self._completer_lock:
            ...
            self.prompt_app = PromptSession(
                lexer=PygmentsLexer(UsqlCliLexer),
...
@click.command()
@click.option("-V", "--version", is_flag=True, help="Output usql's version.")
@click.option("-D", "--database", "dbname", help="Database to use.")
@click.option(
    "-R",
    "--prompt",
    "prompt",
    help='Prompt format (Default: "{0}").'.format(UsqlCli.default_prompt),
)
@click.option(
    "-l",
    "--logfile",
...
def cli(
    ...
    Examples:
      - usql -k <uptycs keyfile> database
        ...
        where database = [global|realtime|timemachine]
    ...
    if database not in ["global", "realtime", "timemachine"]:
    ...
    usql.connect(database)

    usql.logger.debug("Launch Params: \n" "\tdatabase: %r", database)

    #  --execute argument
    if execute:
        try:
            if csv:
                usql.formatter.format_name = "csv"
            elif not table:
                usql.formatter.format_name = "tsv"

            usql.run_query(execute)
            exit(0)
        except Exception as e:
    ...
    if sys.stdin.isatty():
        ...
        try:
            ...
            usql.logger.warning("Unable to open TTY as stdin.")

        if usql.destructive_warning and confirm_destructive_query(stdin_text) is False:
            exit(0)
        try:
            new_line = True
            ...
            usql.run_query(stdin_text, new_line=new_line)
            exit(0)
        except Exception as e:
            click.secho(str(e), err=True, fg="red")
            exit(1)
