...
import platform
import shlex
from usql.uptycs_restcall import ProgrammingError

from usql import __version__
from usql.packages.special import iocommands
from usql.packages.special.utils import format_uptime
...
def list_tables(cur, arg=None, arg_type=PARSED_QUERY, verbose=False):
    ...
    # if verbose and arg:
    #     query = "SELECT sql FROM usql WHERE name LIKE ?"
    #     log.debug(query)
    #     cur.execute(query)
...
def status(cur, **_):
    ...
    # Output the usql client information.
    implementation = platform.python_implementation()
    version = platform.python_version()
    client_info = []
    client_info.append("usql {0},".format(__version__))
